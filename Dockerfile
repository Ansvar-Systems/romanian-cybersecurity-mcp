# ─────────────────────────────────────────────────────────────────────────────
# romanian-cybersecurity-mcp — multi-stage Dockerfile
# ─────────────────────────────────────────────────────────────────────────────
# Build:  docker build -t romanian-cybersecurity-mcp .
# Run:    docker run --rm -p 3000:3000 romanian-cybersecurity-mcp
#
# The image bakes /app/data/dnsc.db at build time. Override DNSC_DB_PATH
...
ENV DNSC_DB_PATH=/app/data/dnsc.db
...
# Bake the pre-built database into the image so /app/data/dnsc.db resolves
...
# We then COPY it into the image at /app/data/dnsc.db (DNSC_DB_PATH). The
# explicit `data/<name>.db` reference is required for the workflow's grep
# `COPY\s+\K(data/\S+\.db)` to match.
COPY data/database.db data/dnsc.db
